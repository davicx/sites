...
class Group {
	...
	public function getGroupActivity($loggedInUser, $group_id) {
		...
		//TYPE 2: DISCUSSION
			...
			//PART 2: Get New Discussions
			//Step 1: Find Last Time Logged in User Visited this page
			$sql = "SELECT * FROM user_analytics WHERE page_url = 'group_discussion.php' 		
				AND group_id = '$group_id' 
				AND user_name = '$loggedInUser'";

			$result_last_page_view = $conn->query($sql) or die(mysqli_error());	
			
			$page_views =  mysqli_num_rows($result_last_page_view);
			
			//Step 2: Logged in User has Not visited this Page so All Discussions are New
			if($page_views == 0) {
				$discussion_new_posts = $discussion_total_posts;

			//Step 3: Select all new Discussions since this time 				
			} else {
				while($row_last_page_view = mysqli_fetch_array($result_last_page_view)) {
					$last_discussion_page_view = $row_last_page_view['last_visit'];	
				...
				$sql = "SELECT post_id FROM posts WHERE post_to = '$group_id' 
					...
					AND post_type = 'discussion'
					AND post_from != '$loggedInUser'
					AND created >= '$last_discussion_page_view' ";

				$result_discussion_new_total = $conn->query($sql) or die(mysqli_error());	
				$discussion_new_posts = mysqli_num_rows($result_discussion_new_total);	
			}	

			$this->groupTotalDiscussions = $discussion_total_posts;	
			$this->groupTotalUnseenDiscussion = $discussion_new_posts;		
