...
if (isset($_POST["post_text_from"]) && (!empty($_POST["post_comment"]))) {	
	...
	if(isset($_POST['board_id'])){ $board_id = $_POST['board_id']; } else { $board_id = 0; }	
	...
	$stmt = $conn->prepare("INSERT INTO posts(master_site, parent, group_id, board_id, post_from, post_to, post_caption, post_url, post_type, article_title, article_text, post_status, updated ) 
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())");
	$stmt->bind_param("siissssssssi", $master_site, $parent, $group_id, $board_id, $post_from, $post_to, $post_comment, $post_url, $post_type, $article_title, $article_text, $post_status);
...
if (isset($_POST['post-photo-upload'])) {
	...
	try {
		...
		if(isset($_POST['post-text-caption'])){ $post_caption = $_POST['post-text-caption']; } else { $post_caption = "A new photo"; } 
		...
		if(isset($_POST['group-id'])){ $group_id = $_POST['group-id']; } else { $group_id = 0; }		
		if(isset($_POST['board-id'])){ $board_id = $_POST['board-id']; } else { $board = 0; }		
...
if (isset($_POST["post_from_video"]) && (!empty($_POST["video_url"]))) {
	...
	//STEP 1: Insert into posts table 	
	$stmt = $conn->prepare("INSERT INTO posts(master_site, group_id, board_id, post_from, post_to, post_url, video_code, post_caption, post_type, post_status, updated, created) VALUES (?,?,?,?,?,?,?,?,?,?, NOW(), NOW())");
	$stmt->bind_param("siissssssi",  $master_site, $group_id, $board_id, $post_from, $post_to, $video_url, $video_code, $video_caption, $post_type, $post_status);
...
if (isset($_POST['post-article-upload'])) {
	...
	try {
		...
		//STEP 3: Insert Post into Database (COULD BE MESSED UP FROM BOARD ID) 
		$stmt = $conn->prepare("INSERT INTO posts( master_site, post_type, post_status, group_id, board_id, post_from, post_to, post_caption, post_url,
			image_url, file_name, file_name_server, article_title, updated, created) 
			VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?, NOW(), NOW())");
			
		$stmt->bind_param("ssiiissssssss", $master_site, $post_type, $post_status, $group_id, $board_id, $post_from, $post_to, $post_caption, $post_url, $image_url, $file_name_full, $file_name_server, $article_title);
