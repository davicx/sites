...
FUNCTIONS A: USER RELATED
	...
	3) Method A3: Get Pending Requests (Friend, Group, List)  
...
FUNCTIONS C: USER SITE ACTIVITY 
	...
	3) Method C3: Update Group Posts to Seen
	4) Method C4: Update Group Discussion to Seen
	5) Method C5: Update New File Activity to Seen
...
class User{
	...
	public function declineFriendRequest($request_from, $logged_in_user) {
		...
		//Update Pending Requests and remove current request 
		if (!($stmt = $conn->prepare("UPDATE pending_requests SET request_is_pending = ? 
			WHERE sent_by = '$request_from' AND sent_to = '$logged_in_user' AND request_type = 'friend'"))) {
			echo "Prepare failed: (" . $conn->errno . ") " . $conn->error;
		}
		...
		//Update Pending Friendship one from friends table 
		...
		//Update Pending Friendship two from friends table 			
	...
	public function declineGroupRequest($decline_group_request_id, $group_id, $logged_in_user) {
		...
		//STEP 1: Update pending request table and remove request 
		if (!($stmt = $conn->prepare("UPDATE pending_requests SET request_is_pending = ? 
		...
		//STEP 2: Delete pending user From group
	...
	//Method C3: Update Group Posts to Seen	
	...
	//Method C4: Update Group Discussion to Seen
	...
	//Method C5: Update New File Activity to Seen
