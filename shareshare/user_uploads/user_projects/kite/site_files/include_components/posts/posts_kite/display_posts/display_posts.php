...
<?php
	...
	//TYPE 1: Individual Board (get all posts with group id same as board)
 	if($display_posts_type ==1) {
		//echo "INDIVIDUAL BOARD ";
		
	//TYPE 2: Kite Home Page (Select Random posts from all the boards you are following)  	
	} else if ($display_posts_type ==2) {
		echo " Kite Home Page";
		
	//TYPE 3: Kite Discover (Select Random posts)
	} else if ($display_posts_type ==3) {
		echo "Kite Discover  ";
	...
	if(isset($_GET["group_id"])) {
		$board_id = base64_decode($_GET['group_id']);	
	} 
	...
	//Type 1: Select All Global Posts 
	/*
	$sql = "SELECT * FROM posts WHERE post_status = '1' AND
		(post_type = 'photo' OR post_type = 'video' OR post_type = 'text' OR post_type = 'article' OR post_type = 'file' ) 
		AND list_id = 0 
		ORDER BY post_id DESC";
	$result = $conn->query($sql) or die(mysqli_error());	
	*/
	//Type 2: Select All From Who I am following (a mix of all the boards I am following)
	
	//Type 3: Select All Posts for a specific board
	/*
	$sql = "SELECT * FROM posts WHERE post_status = '1' AND
		(post_type = 'photo' OR post_type = 'video' OR post_type = 'text' OR post_type = 'article' OR post_type = 'file' ) AND
		post_to = '$post_to' AND list_id = 0 AND group_id = '$board_id'
		ORDER BY post_id DESC";
	$result = $conn->query($sql) or die(mysqli_error());
	*/
	/*
	...
	while($row = mysqli_fetch_array($result)) {		
		...
		$group_id 		= $Current_Post->group_id;
