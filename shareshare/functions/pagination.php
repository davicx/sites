...
if (isset($_POST["fetch_records"]) && (!empty($_POST["fetch_records"]))) {
	$current_post_count = $_POST["current_post_count"];
	$fetch_records = $_POST["fetch_records"];
	...
	$total_posts_found =mysqli_num_rows($result);
	
	//Handle zero posts found 
	
	//total posts found does not equal zero 
	...
	//echo json_encode($posts_json_array);	
	//echo $total_posts_found;
	
	
	
	/* Example
		//$post_1 = array("post_id"=>"1", "post_caption"=>"Post 1", "post_type"=>"Photo");
	//$post_2 = array("post_id"=>"2", "post_caption"=>"Post 2", "post_type"=>"Video");
	//$post_3 = array("post_id"=>"3", "post_caption"=>"Post 3", "post_type"=>"Photo");
	
	//$posts_json_array = array($post_1, $post_2, $post_3);
	*/
...
/*
	$sql = "SELECT * FROM posts WHERE ORDER BY post_id DESC";
	$result = $conn->query($sql) or die(mysqli_error());
	...
	while($row = mysqli_fetch_array($result)) {		
		...
		include('include_components/posts/posts_shareshare/display_posts/individual_post.php');
	} 
?>
*/
