...
FUNCTIONS A: USER RELATED
	...
	3) Method A3: Get Pending Requests (Friend, Group, List) *Double check for accuracy *change pending request table 
...
FUNCTIONS C: USER SITE ACTIVITY 
	1) Method C1: Get New posts Made to User
	2) Method C2: Get New Files Shared with User
	3) Method C3: Get New Friend Requests
	4) Method C4: Get Friend Requests Accepted 
	
	/*
	FUNCTIONS C: SITE ACTIVITY	
	1) Method C1: Get New Group Posts
	2) Method C2: Get New Discussion Posts
	3) Method C3: Get New File Activity
	*Update Methods can be called if user clicks on a notification (which takes them to correct page) 
	...
	7) Method C7: On Login Create a Notification for New Group Discussion and Files Add to Group Icons
	
	1) Method C1: Get New Group Posts
	2) Method C2: Update Group Posts to Seen
	3) Method C3: Get New Discussion Posts
	4) Method C4: Update Group Discussion to Seen
	5) Method C5: Get New File Activity
	6) Method C6: Update New File Activity to Seen
	7) Method C7: Get New Group Followers 
...
FUNCTIONS U: USER ACCOUNT	
	1) Method U1: Register User
	2) Method U2: Register User with Email
	3) Method U3: Request Username  
	4) Method U4: Request Password Send Email (Part 1)
	5) Method U5: Request Password Update to New Password (Part 2)
...
class User{
	...
	public function getUserInfo($userName) {
			...
			$result_posts = mysqli_query($conn, "SELECT * FROM posts WHERE post_to = '$userName' AND post_type = 'item' AND post_status = '1'");
	...
	public function registerUser($user_name, $first_name, $last_name, $email, $pword) {
		...
		$stmt->execute();
		$stmt->close();
		//Insert into User Size (For Wishlist)
	...
	//Method U2: Register User with Email
	public function registerUserEmail($user_name, $first_name, $last_name, $email, $pword, $hash, $request_from_friend) {
		...
		//Step 5: Create new folder on server for new user creation
		if (!file_exists('user_files/user_uploads/'.$username.'')) {
			mkdir('user_files/user_uploads/'.$username.'', 0777, true);
		}
