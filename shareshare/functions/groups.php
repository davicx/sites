...
if (isset($_POST["leave_group_id"]) && (!empty($_POST["logged_in_user"]))) {		
	...
	$sql = "UPDATE group_users SET active_member = 0 WHERE user_name = ? AND group_id = ?";
	$stmt = $conn->prepare($sql);
	$stmt->bind_param('si', $logged_in_user, $group_id);
	if ($stmt->execute()) {
		echo "success";
	} else {
		echo "An error occured leaving the group";
	}
