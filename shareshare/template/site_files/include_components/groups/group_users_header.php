<div id = "header-group-members">	
	<?php 
	for($x = 0; $x < $active_group_users_count; $x++) {
		$current_member_name =  $active_group_users_array[$x]; 
		$current_member_image = getUserImage($current_member_name); 
		$current_member_display_name = getUserFirstName($current_member_name);
		
	?>
		<img src="<?php echo USER_IMAGE . $current_member_image; ?>"  title="<?php echo $current_member_display_name; ?>" alt="<?php echo $current_member_name; ?>" class = "header-member-icon" />
	<?php } ?>
</div>
...
<hr />				
<p class = "text-zero"> Pending Members (<?php echo $pending_group_users_count; ?>) </p>				
