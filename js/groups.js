...
//Function A1: Create Group from Existing User 
$(document).ready(function(){
	$("#js-create-new-group").click(function(){
		...
		var notification_message 	= $('#js-create-group-notification-message').val();
		var notification_link		= $('#js-create-group-notification-link').val();
		var notification_type	 	= $('#js-create-group-notification-type').val();
		var notification_from		= $('#js-create-group-notification-from').val();
		$('#js-create-new-group-error').hide();			
		...
		//STEP 1: Handle No Name 
		if(group_name) {
			
			//STEP 2: Send to Create a Group 
			...
			$('#js-create-new-group-error').show();	
		}
	...
	$(".js-add-users-to-group").click(function(){
		...
		if (user_selected_test == null){
			...
			$.post(CODE_ROOT + 'groups.php', { 
				...
				group_id: group_id, 
				logged_in_user: logged_in_user, 
				group_add_users_selected: group_add_users_selected 
			}, 
			/*
			notification_message: notification_message, 
			notification_link: notification_link, 
...
$(document).ready(function(){
	$(".js-group-leave").click(function(){
		...
		if (confirm('Are you sure you want to leave this Group?')) {
			...
			internalLeaveGroup(leave_group_id, logged_in_user)
...
function internalLeaveGroup(leave_group_id, logged_in_user) {
	...
	function(data){
		$("#js-group_" + leave_group_id).hide();	
	
	});	
}


//FUNCTIONS D: Right Click Code 
//Function D1: Right Click on Group 
$(document).ready(function(){
    $('.js-group-right-click').on('contextmenu', function(e) {
        ...
        window.leave_group_id  = group_id_local; 
    ...
    context.attach('.js-group-right-click', [
        {header: 'Groups'},
        {divider: true},
        ...
        //Delete Group 
        {text: 'Leave Group', action: function(e){
			
			if (confirm('Are you sure you want to leave this Group?')) {
				//alert(logged_in_user + " " + delete_group_id);
				internalLeaveGroup(leave_group_id, logged_in_user);
